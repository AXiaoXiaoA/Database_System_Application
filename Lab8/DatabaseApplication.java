...
class GUI {
    ...
    private JButton reviseButton = new JButton("确认修改");
    ...
    public GUI() {
        ...
        // 修改按钮初始化
        reviseButton.setPreferredSize(new Dimension(100, 30));
        reviseButton.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                // 遍历标记修改的行
                List<Employee> employees = new ArrayList();
                for (Integer i : modifiedRows) {
                    // 获取标记修改的行的数据
                    Employee employee = new Employee();
                    employee.setEMPNO(tableModel.getValueAt(i, 0).toString());
                    employee.setFIRSTNME(tableModel.getValueAt(i, 1).toString());
                    employee.setMIDINIT(tableModel.getValueAt(i, 2).toString());
                    employee.setLASTNAME(tableModel.getValueAt(i, 3).toString());
                    employee.setWORKDEPT(tableModel.getValueAt(i, 4).toString());
                    employee.setPHONENO(tableModel.getValueAt(i, 5).toString());
                    employee.setHIREDATE(tableModel.getValueAt(i, 6).toString());
                    employee.setJOB(tableModel.getValueAt(i, 7).toString());
                    employee.setEDLEVEL(tableModel.getValueAt(i, 8).toString());
                    employee.setSEX(tableModel.getValueAt(i, 9).toString());
                    employee.setBIRTHDATE(tableModel.getValueAt(i, 10).toString());
                    employee.setSALARY(tableModel.getValueAt(i, 11).toString());
                    employee.setBONUS(tableModel.getValueAt(i, 12).toString());
                    employee.setCOMM(tableModel.getValueAt(i, 13).toString());
                    employees.add(employee);
                }
                Database database = Database.getInstance();
                database.reviseTable("TEMPL", employees);
                // 清空修改标记
                modifiedRows.clear();
            }
        });
    ...
    public void createGUI() {
        ...
        ++gbc.gridy;
        rightPanel.add(reviseButton, gbc);
        ...
        table.getModel().addTableModelListener(new TableModelListener() {
            ...
            public void tableChanged(TableModelEvent e) {
                if (e.getType() == TableModelEvent.UPDATE) {
                    modifiedRows.add(e.getFirstRow());
                }
            }
        });
        return table;
...
class Database {
    ...
    /// 修改
    public void reviseTable(String tableName, List<Employee> employees) {
        PreparedStatement pstmt = null;
        try {
            // 初始化sql语句
            String sql = "UPDATE " + tableName + " SET " +
                    "FIRSTNME = ?, MIDINIT = ?, LASTNAME = ?, WORKDEPT = ?, PHONENO = ?, " +
            ...
            for (Employee employee:employees) {
                // 完善sql语句
                if (employee.getFIRSTNME().equals("空") || employee.getFIRSTNME().isEmpty()) {
                    pstmt.setNull(1, Types.CHAR);
                }
                else {
                    pstmt.setString(1, employee.getFIRSTNME());
                }
                if (employee.getMIDINIT().equals("空") || employee.getMIDINIT().isEmpty()) {
                    pstmt.setNull(2, Types.CHAR);
                }
                else {
                    pstmt.setString(2, employee.getMIDINIT());
                }
                if (employee.getLASTNAME().equals("空") || employee.getLASTNAME().isEmpty()) {
                    pstmt.setNull(3, Types.CHAR);
                }
                else {
                    pstmt.setString(3, employee.getLASTNAME());
                }
                if (employee.getWORKDEPT().equals("空") || employee.getWORKDEPT().isEmpty()) {
                    pstmt.setNull(4, Types.CHAR);
                }
                else {
                    pstmt.setString(4, employee.getWORKDEPT());
                }
                if (employee.getPHONENO().equals("空") || employee.getPHONENO().isEmpty()) {
                    pstmt.setNull(5, Types.CHAR);
                }
                else {
                    pstmt.setString(5, employee.getPHONENO());
                }
                if (employee.getHIREDATE().equals("空") || employee.getHIREDATE().isEmpty()) {
                    pstmt.setNull(6, Types.DATE);
                }
                else {
                    pstmt.setDate(6, Date.valueOf(employee.getHIREDATE()));
                }
                if (employee.getJOB().equals("空") || employee.getJOB().isEmpty()) {
                    pstmt.setNull(7, Types.CHAR);
                }
                else {
                    pstmt.setString(7, employee.getJOB());
                }
                if (employee.getEDLEVEL().equals("空") || employee.getEDLEVEL().isEmpty()) {
                    pstmt.setNull(8, Types.SMALLINT);
                }
                else {
                    pstmt.setShort(8,  Short.parseShort(employee.getEDLEVEL()));
                }
                if (employee.getSEX().equals("空") || employee.getSEX().isEmpty()) {
                    pstmt.setNull(9, Types.CHAR);
                }
                else {
                    pstmt.setString(9, employee.getSEX());
                }
                if (employee.getBIRTHDATE().equals("空") || employee.getBIRTHDATE().isEmpty()) {
                    pstmt.setNull(10, Types.DATE);
                }
                else {
                    pstmt.setDate(10, Date.valueOf(employee.getBIRTHDATE()));
                }
                if (employee.getSALARY().equals("空") || employee.getSALARY().isEmpty()) {
                    pstmt.setNull(11, Types.DECIMAL);
                }
                else {
                    pstmt.setBigDecimal(11, (new BigDecimal(employee.getSALARY())));
                }
                if (employee.getBONUS().equals("空") || employee.getBONUS().isEmpty()) {
                    pstmt.setNull(12, Types.DECIMAL);
                }
                else {
                    pstmt.setBigDecimal(12, (new BigDecimal(employee.getBONUS())));
                }
                if (employee.getCOMM().equals("空") || employee.getCOMM().isEmpty()) {
                    pstmt.setNull(13, Types.DECIMAL);
                }
                else {
                    pstmt.setBigDecimal(13, (new BigDecimal(employee.getCOMM())));
                }
                if (employee.getEMPNO().equals("空") || employee.getEMPNO().isEmpty()) {
                    pstmt.setNull(14, Types.CHAR);
                }
                else {
                    pstmt.setString(14, employee.getEMPNO());
                }
                pstmt.executeUpdate();
                ++reviseCount;
            }
            // 执行sql语句
            JOptionPane.showMessageDialog(null, "修改员工数：" + reviseCount, "Success", JOptionPane.PLAIN_MESSAGE);
            pstmt.close();
        } catch (NumberFormatException e) {
            Database.getInstance().handleNumberFormatException(e);
        } catch (IllegalArgumentException e) {
